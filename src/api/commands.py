...
def setup_commands(app):
    ...
    def insert_test_data():
        ...
        for p_info in products_data:
            ...
            existing_prod = Product.query.filter_by(name=p_info["name"]).first()
        ...
        if not sample_order:
            prod1 = Product.query.filter_by(name="Smartwatch ChronoFit Ultra").first()
            prod2 = Product.query.filter_by(name="Botella Térmica Inox 750ml").first()
            ...
            if prod1 and prod2:
                initial_order = Order(
                    ...
                    items=[
                        OrderItem(product_id=prod1.id, quantity=1, unit_price=prod1.price),
                        OrderItem(product_id=prod2.id, quantity=2, unit_price=prod2.price)
                    ]
                )
